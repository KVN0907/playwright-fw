...
export class Document360ProjectCreationPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.getStartedButton = page.locator('app-use-case-tile').filter({ hasText: 'API documentation' }).getByRole('button', { name: 'Get started' });
    ...
    async selectApiSetupMethod(setupType: 'sample' | 'upload' | 'url', apiUrl?: string) {
        ...
        switch (setupType) {
            ...
            case 'upload':
                ...
                const isFileInputVisible = await this.fileInput.isVisible({ timeout: 2000 }).catch(() => false);
            ...
            default:
                throw new Error(`Unsupported API setup type: ${setupType}. Use 'sample', 'upload', or 'url'.`);
        }
    }
    ...
    async verifyStepIndicator(expectedStep: string) {
        ...
        if (stepText && expectedStep) {
            await this.verifyText(this.page.locator(`text=${expectedStep}`), expectedStep, "Step indicator");
        }
    }
    ...
    async verifyPrivateAccessSelected() {
        ...
        const privateRadio = this.page.locator('input[value="private"], [data-value="private"]').first();
