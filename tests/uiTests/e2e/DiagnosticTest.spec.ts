...
test.describe('Diagnostic Tests', () => {
  test('Check page state and authentication', async ({ page }) => {
    ...
    const welcomeTexts = bannerElements.filter(text => 
    ...
    Log.info(`Page contains login form: ${pageContent.includes('form') && pageContent.includes('password')}`);
    Log.info(`Page contains authentication elements: ${pageContent.includes('auth') || pageContent.includes('login')}`);
