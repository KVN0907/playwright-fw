...
test.describe('Document360 Swagger Pet Store API Endpoint Publishing', () => {
  ...
  test('User can interact with Try It functionality on published API endpoint', async () => {
    ...
    // And user clicks the Try it & see response button
    await publishedSitePage.clickTryItButton();

    // Then the API response should be validated
    await publishedSitePage.validateApiResponse();
