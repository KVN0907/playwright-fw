...
export class OrganizationValidator extends BaseValidator {
  ...
  static validateBusinessRules(org: OrganizationDetails): void {
    ...
    if (org.type === 'enterprise' && org.settings.maxMembers < 50) {
      throw new Error('Enterprise organizations must support at least 50 members');
    }

    // Individual organizations should have limited members
    if (org.type === 'individual' && org.settings.maxMembers > 5) {
      throw new Error('Individual organizations cannot exceed 5 members');
    }
