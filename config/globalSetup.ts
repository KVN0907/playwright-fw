...
import { AuthenticationManager } from '../src/lib/auth/AuthenticationManager';
...
function loadEnvWithDebug() {
  ...
  const envFile = process.env.NODE_ENV ? `${process.env.NODE_ENV}.env` : 'dev.env';
  ...
  if (!fs.existsSync(envPath)) {
    ...
    // Fallback to dev.env file
    const fallbackPath = path.resolve(__dirname, './environments/dev.env');
